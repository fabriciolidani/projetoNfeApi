...
app.post('/personEsse', async (req, res) => {
  ...
  try {
    await InformacoesManifesto.create(infonfe)
...
app.post('/consultaUltNSU/:nsuNfe', async (req, res) => {
  const nsuNfe = req.params.nsuNfe;
  console.log(nsuNfe)
  try {
    const distribuicao = new DistribuicaoDFe({
      pfx: fs.readFileSync('./MILENGENHARIA.pfx'),
      passphrase: '20202020',
      cnpj: '17828802000197',
      cUFAutor: '43',
      tpAmb: '1',
    ...
    const consulta = await distribuicao.consultaUltNSU('000000000009760')
    ...
    if (objetoResXmlJson["soap:Envelope"] &&
      ...
      for (let i = 0; i < objetoResXmlJson["soap:Envelope"]["soap:Body"]["nfeDistDFeInteresseResponse"]["nfeDistDFeInteresseResult"]["retDistDFeInt"]["loteDistDFeInt"]["docZip"].length; i++) {
        ...
        /**
        Possíveis tags que o xml pode conter:
  
        //resNFe temos apenas um resumo da nota
        //resEvento temos apenas um resumo do evento
        //nfeProc temos a nota completa inclusive com a assinatura e protocolo de autorização
        //procEventoNFe temos o evento completo, ou seja, a solicitação e o retorno da SEFAZ acusando que o mesmo foi registrado e vinculado a nota. */
...
app.post('/consultaNSU/:nsu', async (req, res) => {
...
app.post('/consultaChNFe/:chNfe', async (req, res) => {
  ...
  try {
    ...
    // O XML completo da NF-e está na variável "xml"
    console.log(xml)
...
app.post('/manifestarNFE/:valoresSelecionados', async (req, res) => {
  ...
  try {
    ...
    // Apenas uma NFe para manifestar
    var qtdEventos = 0;
    if (lote.length == 1484866) {
      ...
      Promise.all(valores.map(async (element) => {
        ...
        .then(() => {
          console.log(resultadosNfeCompleta)
...
app.get('/person', async (req, res) => {
  try {
    (async () => {
      try {
        const response = await axios.get('http://184.72.206.2:3000/person')

        var teste = response.data
        teste.forEach(objetoArray => {
...
// Consulta a tabela informcoesManifesto
app.get('/informacoesManifesto/:id', async (req, res) => {
...
// Atualiza a tabela informacoesManifesto
app.patch('/informacoesManifesto/:id', express.json(), async (req, res) => {
